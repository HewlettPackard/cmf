...
def _require(data, key):
    ...
    val = data.get(key, "")
    if not val:
        pytest.fail(f"config.json: '{key}' is not set. Required for sshremote backend.")
    return val


def test_cmf_init_sshremote(cmf_server_url):
    print("-------------------------------Test Case Name: cmf init sshremote----------------------------------")
    data = _get_config()
    path     = _require(data, "ssh_path")
    user     = _require(data, "ssh_user")
    port     = data.get("ssh_port", "22") or "22"
    password = _require(data, "ssh_password")
