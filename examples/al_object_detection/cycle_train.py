...
def main():
    ...
    _ = logger.create_context("Train", {"Selected": "5"})
    _ = logger.create_execution("Train" + "-"+ str(args.cycle), {"seed": str(args.seed), "env":env_info_dict}, create_new_execution=False)

    logger.log_dataset(args.labeled, "input", {"type":"numpy", "labeled":"True"})
    logger.log_dataset(args.unselected, "input", {"type":"numpy", "labeled":"False"} )
    ...
    logger.log_model(args.model, "output", model_framework="mmdet", model_type="RetinaNet", model_name="resnet50", custom_properties=model_dict)
