...
def main():
    ...
    model_dict = cfg.model.copy()
    model_dict.pop("pretrained")

    #CMF Logging
    logger = cmf.Cmf("mlmd", "active_learning", graph=True)
    _ = logger.create_context("Cycle_Select", {"Strategy": "Random"})
    _ = logger.create_execution("Cycle_Select_with_5_percent" + "-" + str(args.cycle), {"config":Config, "env": env_info_dict})
    _ = logger.log_model(args.model, "input", model_framework="mmdet", model_type="RetinaNet", model_name="resnet50")
    ...
    X_L_next, X_U, uncertanity_vals = update_X_L(uncertainty, X_all, X_L, cfg.X_S_size)

    # save next cycle labeled and unlesected lists
    np.save(args.labeled_next, X_L_next)
    np.save(args.unselected, X_U)
    slice_cycle = int(args.cycle) + 1

    label_next = logger.log_dataset(args.labeled_next, "output", {"cycle": str(slice_cycle)})
    _ = logger.log_dataset(args.unselected, "output", {"cycle": str(slice_cycle)})

    '''
    CMF - Create a Dataslice
    '''
    dataslice: cmf.Cmf.DataSlice = logger.create_dataslice(name="slice_"+str(slice_cycle))
    for i in X_L_next:
        ...
        '''CMF - Add images to the dataslice'''
        dataslice.add_data(slice_path, {"Uncertinity": "-1", "cluster_distance": str(random.random())})
    
    '''CMF - Commit the Dataslice'''
    slice = dataslice.commit(custom_properties = uncertanity_vals)

    '''CMF - Link it to the derived artifact'''
