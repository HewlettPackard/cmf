...
async def display_exec_lineage(request: Request, pipeline_name: str, uuid: str):
    ...
    if os.path.exists(server_store_path):
        query = cmfquery.CmfQuery(server_store_path)
        if (pipeline_name in query.get_pipeline_names()):
            response = await async_api(query_exec_lineage, server_store_path, pipeline_name, dict_of_exe_ids, uuid)
    else:
        response = None
    return response
...
async def display_tree_lineage(request: Request,uuid, pipeline_name: str):
    ...
    if os.path.exists(server_store_path):
        query = cmfquery.CmfQuery(server_store_path)
        if (pipeline_name in query.get_pipeline_names()):
            response = await async_api(query_tangled_lineage, server_store_path, pipeline_name, dict_of_exe_ids,uuid)
    return response
...
async def display_arti_tree_lineage(request: Request, pipeline_name: str)-> List[List[Dict[str, Any]]]:
    ...
    if os.path.exists(server_store_path):
        query = cmfquery.CmfQuery(server_store_path)
        if (pipeline_name in query.get_pipeline_names()):
            response = await async_api(query_artifact_tree_lineage, server_store_path, pipeline_name, dict_of_art_ids)        
    return response
