...
class Cmf(object):
    ...
    def __init__(self, filename: str = "mlmd",
            pipeline_name = "", custom_properties :{} = {}, graph:bool = False):
        ...
        self.graph = graph
        self.parent_context = get_or_create_parent_context(store=self.store, pipeline=pipeline_name, custom_properties=custom_properties)
        if(graph):
    ...
    def create_execution(self, execution_type :str,
       ...
       # self.execution_command = re.sub('\W+','_', temp_str)
        ...
        if self.graph:
            self.driver.create_execution_node(self.execution_name, self.child_context.id, self.parent_context.id, str(sys.argv), self.execution.id,  custom_props)
    ...
    def log_dataset(self, url :str, event :str, custom_properties :{} = None) -> mlpb.Artifact:
        ...
        if self.graph:
            self.driver.create_dataset_node(name, url, uri, event, self.execution.id, self.parent_context.id, custom_props)
            if event.lower() == "input":
                self.input_artifacts.append( {"Name":name, "Path":url, "URI":uri, "Event":event.lower(), "Execution_Name" : self.execution_name,
                                              "Type" : "Dataset", "Execution_Command": self.execution_command, "Pipeline_Id":self.parent_context.id })
                self.driver.create_execution_links(uri, name, "Dataset", self.execution.id, self.parent_context.id)
            else :
                child_artifact = {"Name":name, "Path":url, "URI":uri, "Event":event.lower(), "Execution_Name" : self.execution_name,
                                               "Type" : "Dataset", "Execution_Command": self.execution_command ,"Pipeline_Id":self.parent_context.id}
    ...
    def log_model(self, path :str, event :str, model_framework: str, model_type: str, model_name: str, custom_properties: {} = {}) -> object:
        ...
        if self.graph:
            self.driver.create_model_node(model_uri, uri, event, self.execution.id, self.parent_context.id, custom_props)
            if event.lower() == "input":

                self.input_artifacts.append( {"Name":model_uri, "URI":uri, "Event":event.lower(), "Execution_Name" : self.execution_name,
                    "Type" : "Model", "Execution_Command": self.execution_command, "Pipeline_Id":self.parent_context.id })
           #     self.input_artifacts.append( {"Name":name,''' "Path":model_uri,''' "URI":uri, "Event":event.lower(), "Execution_Name" : self.execution_name,
            #                                                     "Type" : "Model", "Execution_Command": self.execution_command })
                ...
                child_artifact = {"Name":model_uri, "URI":uri, "Event":event.lower(), "Execution_Name" : self.execution_name,
                                                                  "Type" : "Model", "Execution_Command": self.execution_command, "Pipeline_Id":self.parent_context.id }
                self.driver.create_artifact_relationships(self.input_artifacts, child_artifact, self.execution_label_props)
    ...
    def log_execution_metrics(self, metrics_name :str, custom_properties: {} = None) -> object:
        ...
        if self.graph:
            ...
            self.driver.create_metrics_node(metrics_name,  uri, "output", self.execution.id, self.parent_context.id, custom_props)
            child_artifact = {"Name":metrics_name,  "URI":uri, "Event":"output", "Execution_Name" : self.execution_name,
                                                                  "Type" : "Metrics", "Execution_Command": self.execution_command, "Pipeline_Id":self.parent_context.id }
            self.driver.create_artifact_relationships(self.input_artifacts, child_artifact, self.execution_label_props)
        return metrics
    ...
    def commit_metrics(self, metrics_name :str):
        ...
        if self.graph:
            ...
            self.driver.create_metrics_node(name,  uri, "output", self.execution.id, self.parent_context.id, custom_props)
    ...
    class dataslice(object):
        ...
        def commit(self, custom_props:{}=None):
            ...
            if (existing_artifact and len(existing_artifact) != 0):
                ...
                artifact = link_execution_to_input_artifact(store=self.writer.store,
