...
class GraphDriver:
    ...
    def create_pipeline_node(self, name: str, uri: int, props={}):
        pipeline_syntax = self._create_pipeline_syntax(name, props, uri)
        with self.driver.session() as session:
            pipeline = session.write_transaction(self._run_transaction, pipeline_syntax)

    def create_stage_node(self, name: str, parent_id: int, stage_id: int, props={}):
        stage_syntax = self._create_stage_syntax(name, props, stage_id, parent_id)
        pc_syntax = self._create_parent_child_syntax("Pipeline", "Stage", parent_id, stage_id, parent_id, "contains")
        with self.driver.session() as session:
            stage = session.write_transaction(self._run_transaction, stage_syntax)
    ...
    def create_execution_node(self, name: str, parent_id: int, pipeline_id: int, command: str, execution_id: int,
        ...
        pc_syntax = self._create_parent_child_syntax("Stage", "Execution", parent_id, execution_id, pipeline_id, "runs")
        with self.driver.session() as session:
            execution = session.write_transaction(self._run_transaction, execution_syntax)
            relation = session.write_transaction(self._run_transaction, pc_syntax)

    def create_dataset_node(self, name: str, path: str, uri: str, event: str, execution_id: int, pipeline_id: int,
                            custom_properties={}):

        dataset_syntax = self._create_dataset_syntax(name, path, uri, event, execution_id, pipeline_id,
                                                     custom_properties)
        pc_syntax = self._create_execution_artifacts_link_syntax("Execution", "Dataset", execution_id, uri, name, pipeline_id,  event)
        with self.driver.session() as session:
            execution = session.write_transaction(self._run_transaction, dataset_syntax)
            relation = session.write_transaction(self._run_transaction, pc_syntax)

    def create_model_node(self, name: str, uri: str, event: str, execution_id: str, pipeline_id: int,
                          custom_properties={}):

        model_syntax = self._create_model_syntax(name, uri, event, execution_id, pipeline_id, custom_properties)
        pc_syntax = self._create_execution_artifacts_link_syntax("Execution", "Model", execution_id, uri, name, pipeline_id, event)
        with self.driver.session() as session:
            execution = session.write_transaction(self._run_transaction, model_syntax)
            relation = session.write_transaction(self._run_transaction, pc_syntax)

    """
    def create_model_node(self, name:str, path:str, uri:str, event:str, execution_id:str, pipeline_id:int,custom_properties={}):
    ...
    def create_metrics_node(self, name: str, uri: str, event: str, execution_id: str, pipeline_id: int,
                            custom_properties={}):

        metrics_syntax = self._create_metrics_syntax(name, uri, event, execution_id, pipeline_id, custom_properties)
        pc_syntax = self._create_execution_artifacts_link_syntax("Execution", "Metrics", execution_id, uri, name, pipeline_id, event)
        with self.driver.session() as session:
            execution = session.write_transaction(self._run_transaction, metrics_syntax)
            relation = session.write_transaction(self._run_transaction, pc_syntax)
    ...
    def create_artifact_relationships(self, parent_artifacts, child_artifact, relation_properties):
        ...
        for k in parent_artifacts:
            ...
            pc_syntax = self._create_parent_child_artifacts_syntax(parent_artifact_type, child_artifact_type,
                                                                   ...
                                                                   child_name, pipline_id, relation, relation_properties)
            with self.driver.session() as session:
                relation = session.write_transaction(self._run_transaction, pc_syntax)


    def create_execution_links(self, parent_artifact_uri, parent_artifact_name, parent_artifact_type, execution_id, pipeline_id):

        parent_execution_query = "MATCH (n:{}".format(
                parent_artifact_type) + "{uri: '" + parent_artifact_uri + "', pipeline_id:'" + str(pipeline_id) +"'}) <-[:output]-(execution) return execution.uri as uri"
    ...
    @staticmethod
    def _create_dataset_syntax(name: str, path: str, uri: str, event: str, execution_id: int, pipeline_id: int,
                               custom_properties):
    ...
    @staticmethod
    def _create_model_syntax(name: str, uri: str, event: str, execution_id: int, pipeline_id: int, custom_properties):
        custom_properties["Name"] = name
        custom_properties["uri"] = uri
        # custom_properties["execution_id"] = str(execution_id)
        custom_properties["pipeline_id"] = str(pipeline_id)
    ...
    def _create_parent_child_syntax(parent_label: str, child_label: str, parent_name: str, child_name: str,
        ...
        if relation.lower() == "input":
            ...
            parent_child_syntax = "MATCH (a:{}), (b:{}) WHERE a.uri = '{}' AND b.uri = '{}' AND a.pipeline_id = '{}' AND b.pipeline_id = '{}'\
                               MERGE (a)<-[r:{}]-(b) \
                               RETURN type(r)".format(parent_label, child_label, parent_name, child_name, str(pipeline_id), str(pipeline_id), relation)
        else:
            parent_child_syntax = "MATCH (a:{}), (b:{}) WHERE a.uri = '{}' AND b.uri = '{}' AND a.pipeline_id = '{}' AND b.pipeline_id = '{}' \
                               MERGE (a)-[r:{}]->(b) \
                               RETURN type(r)".format(parent_label, child_label, parent_name, child_name, pipeline_id, pipeline_id, relation)
    ...
    def _create_execution_artifacts_link_syntax(parent_label: str, child_label: str, parent_uri: str, child_uri: str,
        ...
        if relation.lower() == "input":
            ...
            parent_child_syntax = "MATCH (a:{}), (b:{}) WHERE a.uri = '{}' AND b.uri = '{}' AND b.Name = '{}' AND a.pipeline_id = '{}' AND b.pipeline_id = '{}'\
                               MERGE (a)<-[r:{}]-(b) \
                               RETURN type(r)".format(parent_label, child_label, parent_uri, child_uri, child_name, str(pipeline_id), str(pipeline_id),
                                                      relation)
        else:
            parent_child_syntax = "MATCH (a:{}), (b:{}) WHERE a.uri = '{}' AND b.uri = '{}' AND b.Name = '{}' AND a.pipeline_id = '{}' AND b.pipeline_id = '{}'\
                               MERGE (a)-[r:{}]->(b) \
    ...
    def _create_execution_link_syntax(parent_label: str, child_label: str, parent_uri: str, child_uri: str,
        ...
        #  parent_child_syntax_1 = "MATCH (a:{}), (b:{}) WHERE a.Name = '{}' AND b.Name = '{}' ".format(parent_label, child_label, parent_name, child_name)
        ...
        parent_child_syntax = parent_child_syntax_1 + parent_child_syntax_2 + parent_child_syntax_3 + parent_child_syntax_4
    ...
    def _create_parent_child_artifacts_syntax(parent_label: str, child_label: str, parent_uri: str, child_uri: str,
        ...
        parent_child_syntax_1 = "MATCH (a:{}), (b:{}) WHERE a.uri = '{}' AND a.Name = '{}' AND a.pipeline_id = '{}' AND b.pipeline_id = '{}' AND b.Name = '{}' AND b.uri = '{}' ".format(
            parent_label, child_label, parent_uri, parent_name, str(pipeline_id), str(pipeline_id), child_name, child_uri)
        parent_child_syntax_2 = "MERGE (a)-[r:{}".format(relation)
        parent_child_syntax_3 = "{"
        for k, v in relation_properties.items():
