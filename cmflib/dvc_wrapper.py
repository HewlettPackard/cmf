...
# Checkout master branch with quiet option and switch branch with -b option
def git_checkout() -> str:
    commit = ""
    try:
        process = subprocess.Popen(['git', 'checkout', '-q', '-b', 'master'],
                                   stdout=subprocess.PIPE,
                                   universal_newlines=True)
        output, errs = process.communicate(timeout=60)
        commit = output.strip()
...
# Check whether git is initialised or not
def git_status() -> str:
    commit = ""
    try:
