...
const renderChart = (data, options = {}) => {
  ...
  return `
      ...
      ${tangleLayout.nodes
        .map(
          n => `
          <path class="selectable node" data-id="${n.id}" stroke="black" stroke-width="8" d="M${
            n.x
          } ${n.y - n.height / 2} L${n.x} ${n.y + n.height / 2}"/>
          <path class="node" stroke="white" stroke-width="4" d="M${
            n.x
          } ${n.y - n.height / 2} L${n.x} ${n.y + n.height / 2}"/>
          <text class="selectable" data-id="${n.id}" x="${n.x +
            4}" y="${n.y -
            n.height / 2 -
            4}" stroke="${
            options.background_color
...
const ExecutionTree = ({ data }) => {
  useEffect(() => {
    ...
    //const chart = '<h1>Blog Title</h1><p>The blog content goes here.</p>';
...
//    const reactElements = ReactHtmlParser(chart);
    ...
    return () => {
      // Clean up on unmount
       chartContainer.removeChild(svgElement);
    };
  }, [data]);
